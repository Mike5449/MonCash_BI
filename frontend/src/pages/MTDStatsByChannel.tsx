...
import { useMtdStatsByMonthAndChannel } from "../hooks/useAnalytics"
...
function buildPivot(rows: any[], metric: Metric): Pivot {
  ...
  for (const r of rows) map.set(`${r.CHANNEL}|${r.MONTH}`, r)

  const data = channels.map(c => {
    const cells = months.map(m => Number((map.get(`${c}|${m}`) || {})[metric] ?? 0))
...
export default function MTDStatsByChannel() {
  ...
  const { data: rows, isLoading, isFetching, refetch } = useMtdStatsByMonthAndChannel({
  ...
  const exportToExcel = async () => {
    try {
      await downloadXlsxStream(
        '/customers/mtd-stats-by-month-channel/export.xlsx',
        {
        ...
        `MTD_by_Month_Channel_${submitted.startMonth}_to_${submitted.endMonth}.xlsx`,
      ...
      <div style={{
        ...
        <div>
          <div style={{
            ...
            Customer · Month-to-date by channel
          </div>
          <h1 style={{
            margin: 0,
            ...
            Month-to-Date Stats by Channel
          </h1>
          <p style={{
            ...
            Aggregate by month × CHANNEL — {monthLabel(submitted.startMonth)} → {monthLabel(submitted.endMonth)}
        ...
        {busy && allRows.length === 0 ? (
          ...
          <>
            ...
            {activeMetric === 'SUBS' && (
              <PivotTable
                title="Subscribers"
                subtitle="Distinct MSISDN per month (attribution canal primaire)"
...
// ── Pivot Table component — CHANNEL en lignes, mois en colonnes ──
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          {pivot.rows.length} channel{pivot.rows.length > 1 ? 's' : ''} · {pivot.months.length} month{pivot.months.length > 1 ? 's' : ''}
        </span>
      </div>
