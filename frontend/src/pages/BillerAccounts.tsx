...
import { Building, Download, Search, RefreshCw } from "lucide-react"
import "../premium.css"

export default function BillerAccounts() {
  const { data: accounts, isLoading, refetch } = useBillerAccounts({ limit: 100 })
    ...
    <DashboardLayout>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <span style={{ display: 'block', fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', color: 'var(--mc-red)' }}>Biller Repository</span>
        ...
        <div className="search-container" style={{ flex: 1, marginBottom: 0, height: '44px' }}>
           ...
           <input type="text" className="search-input" placeholder="Search billers by name, ID or shortcode..." />
        </div>
      </div>
      ...
      <div className="table-card animate-fade-in" style={{ marginTop: '24px' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--mc-border)', background: '#f8fafc' }}>
           <h3 style={{ fontSize: '13px', fontWeight: '800', margin: 0, textTransform: 'uppercase', color: '#64748b' }}>Registered Biller Partners (Top 100)</h3>
        </div>
          ...
          <table className="mc-table" style={{ minWidth: '1300px' }}>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '100px', color: '#94a3b8' }}>No biller accounts found.</td></tr>
              ) : accounts?.map((a, idx) => (
                <tr key={idx}>
