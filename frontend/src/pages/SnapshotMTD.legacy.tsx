     ...
     <div ref={pageRef} style={{ background: 'var(--mc-bg)', padding: '4px' }}>
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h1 className="page-title" style={{ margin: 0 }}>MonCash KPIs Snapshot · MTD</h1>
            <span style={{ background: '#1e293b', color: 'white', fontSize: '10px', fontWeight: '800', padding: '4px 8px', borderRadius: '6px', letterSpacing: '0.5px' }}>MONTH-ON-MONTH</span>
          </div>
          <p className="page-subtitle" style={{ marginTop: '6px' }}>
            MTD courant vs MTD mois précédent (même jour-du-mois), par TR_TYPE.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '24px' }}>
          <PeriodBadge label="Previous MTD" start={periods.prevStart} end={periods.prevEnd} accent="#059669" />
          <PeriodBadge label="Current MTD" start={periods.currStart} end={periods.currEnd} accent="#2563eb" bold />
        </div>
      </div>
...
function PeriodBadge({ label, start, end, accent, bold = false }: any) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
      <span style={{ fontSize: '10px', fontWeight: '800', textTransform: 'uppercase', color: accent, letterSpacing: '0.5px' }}>{label}</span>
      <span style={{ fontSize: bold ? '13px' : '12px', fontWeight: bold ? '900' : '700', color: 'var(--mc-text-main)', fontVariantNumeric: 'tabular-nums', marginTop: '2px', whiteSpace: 'nowrap' }}>
        {start} → {end}
      </span>
    </div>
  )
