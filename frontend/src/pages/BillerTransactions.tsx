...
import {
  ...
  Smartphone, CheckCircle, UserSearch, Receipt,
} from "lucide-react"
import { DashboardLayout } from "../components/Layout/DashboardLayout"
import { useBillerTransactions, useBillerTransactionsSummary } from "../hooks/useAnalytics"
...
export default function BillerTransactions() {
  ...
  // Status filter — default 'Completed' (backend's legacy behaviour), 'all' bypasses the filter
  ...
  const apply = () => {
    ...
    // 'all' → send the wildcard; specific value → send as a single-element list
    setStatuses(localStatus === 'all' ? ['*'] : [localStatus])
  }
  const clearFilters = () => {
    setPreset('30')
    setLocalStart(minusDays(30)); setLocalEnd(todayMinus1())
    setBillerIdsInput(""); setMsisdnsInput(""); setLocalChannel("")
    setLocalStatus("Completed")
    setStart(minusDays(30)); setEnd(todayMinus1())
    setBillerIds(undefined); setMsisdns(undefined); setChannels(undefined)
  ...
  // ── Fetches ──
  const { data: transactions, isLoading, isFetching, refetch: refetchRows } = useBillerTransactions({
    startDate: start, endDate: end,
    limit: DISPLAY_LIMIT,
    billerIds, msisdns, channels, statuses,
  })
  const { data: summaryData, isFetching: summaryFetching, refetch: refetchSummary } = useBillerTransactionsSummary({
    startDate: start, endDate: end,
    billerIds, msisdns, channels, statuses,
  })
  ...
  const summary = useMemo(() => ({
    ...
    UNIQUE_BILLERS: Number((summaryData as any)?.UNIQUE_BILLERS ?? 0),
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r: any) =>
      ...
      String(r.ACCOUNT_ID ?? '').toLowerCase().includes(s) ||
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await fetch(`${OpenAPI.BASE}/billers/transactions/export.xlsx?${qs.toString()}`)
      ...
      a.href = dl
      a.download = `Biller_Transactions_${start}_to_${end}.xlsx`
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{
        ...
        <div>
          <div style={{
            ...
            Biller · Customer bill payments · Collection log
          </div>
          <h1 style={{
            margin: 0,
          ...
          <p style={{
            ...
            Daily payments from customers to billers — subscriber (MSISDN) is the debit party, biller is the credit party.
          </p>
        </div>
      ...
      {/* ── KPI STRIP — 5 cards (placed above filters) ── */}
      <div style={{
        display: 'grid',
        ...
        <KpiCard label="Unique subs"     value={summaryFetching ? '—' : fmtNum(summary.UNIQUE_SUBS)}    accent="var(--data-subs)"    sub="distinct MSISDN paying bills" />
        <KpiCard label="Unique billers"  value={summaryFetching ? '—' : fmtNum(summary.UNIQUE_BILLERS)} accent="var(--data-volume)"  sub="distinct biller IDs collected" />
      ...
      <div style={{
        ...
        <Field label="Biller IDs">
          <TextInputWithIcon
            ...
            value={billerIdsInput}
            onChange={setBillerIdsInput}
            onEnter={apply}
            placeholder="314, 318, 322…"
            width={180}
          />
        </Field>
        ...
        <div style={{
          ...
          {hasFilters && (
            <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'flex-end', maxWidth: '380px' }}>
              {billerIds && (
                <Tag>{billerIds.length} biller{billerIds.length > 1 ? 's' : ''}</Tag>
              )}
              {msisdns && (
                <Tag>{msisdns.length} MSISDN{msisdns.length > 1 ? 's' : ''}</Tag>
              )}
              {channels && (
                <Tag>{channels[0]}</Tag>
              )}
              {isCustomStatus && (
                <Tag>{statuses?.[0] === '*' ? 'All statuses' : `Status: ${statuses?.[0]}`}</Tag>
              )}
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
            <Receipt size={13} strokeWidth={1.75} color="var(--text-tertiary)" />
            <span style={{
              fontSize: 'var(--fs-micro)', fontWeight: 600,
              color: 'var(--text-tertiary)',
          ...
          <div style={{
            ...
            <input
              type="text"
              placeholder="Filter tx ID, MSISDN, biller ID/name, channel…"
              value={search}
            ...
            <thead style={{ background: 'var(--surface-muted)' }}>
              <tr>
                {[
                  ...
                  { label: 'Biller ID',      align: 'left'  },
                  { label: 'Biller name',    align: 'left'  },
            ...
            <tbody>
              {busy && rows.length === 0 ? (
                <tr><td colSpan={9} style={{ padding: 'var(--space-16) 0', textAlign: 'center', color: 'var(--text-tertiary)' }}>
                  ...
                  <div style={{ fontSize: 'var(--fs-body)', color: 'var(--text-secondary)' }}>Loading biller collections…</div>
                </td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ padding: 'var(--space-16) 0', textAlign: 'center', color: 'var(--text-tertiary)' }}>
                  <XCircle size={24} strokeWidth={1.5} opacity={0.4} />
                  <div style={{ marginTop: 'var(--space-3)', fontSize: 'var(--fs-body)', color: 'var(--text-secondary)' }}>
                    {rows.length === 0 ? "No bill payment for this period and filter combination." : "No row matches the search."}
                  </div>
                </td></tr>
                ...
                return (
                  <tr key={(tr.TRANSACTIONID ?? '') + idx} style={{ borderBottom: '1px solid var(--border-faint)' }}>
                    ...
                    <td style={{
                      padding: 'var(--space-2) var(--space-4)',
                      color: tr.BILLER_NAME ? 'var(--text-primary)' : 'var(--text-muted)',
                      fontWeight: 500,
                      maxWidth: '220px',
                      whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                    }}
                      title={tr.BILLER_NAME || ''}>
                      {tr.BILLER_NAME || '—'}
                    </td>
                    <td style={{
                      padding: 'var(--space-2) var(--space-4)',
