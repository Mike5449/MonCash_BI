...
import { useDailyStatsByChannel } from "../hooks/useAnalytics"
...
function buildPivot(rows: any[], metric: Metric): Pivot {
  ...
  const channels = Array.from(new Set(rows.map(r => String(r.CHANNEL || '')).filter(Boolean))).sort()
  const map = new Map<string, any>()
  for (const r of rows) map.set(`${r.CHANNEL}|${normDate(r.TRANSACTION_DATE)}`, r)

  const pivotRows = channels.map(c => {
    const cells = dates.map(dt => Number((map.get(`${c}|${dt}`) || {})[metric] ?? 0))
    return { dim: c, cells, total: cells.reduce((s, v) => s + v, 0) }
  })
  const colTotals  = dates.map((_, i) => pivotRows.reduce((s, r) => s + r.cells[i], 0))
  const grandTotal = colTotals.reduce((s, v) => s + v, 0)
  return { dates, rows: pivotRows, colTotals, grandTotal }
}

export default function DailyStatsByChannel() {
  ...
  const { data: rows, isLoading, isFetching, refetch } = useDailyStatsByChannel({
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const res = await fetch(`${OpenAPI.BASE}/customers/daily-stats-by-channel/export.xlsx?${qs.toString()}`)
      ...
      a.href = dl
      a.download = `Daily_Stats_by_Channel_${submittedFilters.startDate}_to_${submittedFilters.endDate}.xlsx`
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{
            ...
            Volume, subs, value and revenue aggregated by day × CHANNEL · {submittedFilters.startDate} → {submittedFilters.endDate}
          </p>
        </div>
      ...
      <div style={{
        ...
        {/* Day-of-week filter — client-side */}
        ...
        {/* TR_TYPE filter — restrict which products feed the per-channel aggregates */}
      ...
      {/* ── METRIC TABS + PIVOT — CHANNEL en lignes, dates en colonnes ── */}
        ...
        {busy && allRows.length === 0 ? (
          ...
          <>
            ...
            {activeMetric === 'SUBS' && (
              <PivotTable
                title="Subscribers"
                subtitle="Distinct MSISDN per day (attribution canal primaire)"
...
// ── Pivot Table component — CHANNEL en lignes, dates en colonnes ──
...
const DAY_COLORS: Record<number, { bg: string; color: string; border: string }> = {
  0: { bg: '#cffafe', color: '#155e75', border: '#67e8f9' },
  ...
  5: { bg: '#fee2e2', color: '#991b1b', border: '#fca5a5' },
  6: { bg: '#fce7f3', color: '#9d174d', border: '#f9a8d4' },
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          {pivot.rows.length} channel{pivot.rows.length > 1 ? 's' : ''} · {pivot.dates.length} day{pivot.dates.length > 1 ? 's' : ''}
        </span>
      </div>
