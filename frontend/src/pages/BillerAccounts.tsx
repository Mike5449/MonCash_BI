...
import { Building2, Download, Search, RefreshCw, Calendar, X, Activity } from "lucide-react"
import "../premium.css"
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{
            ...
            Organizational partner registry for utility products and services — one row per unique biller shortcode.
          </p>
        </div>
              ...
              {busy ? (
                <tr>
                  <td colSpan={9} style={{ padding: 'var(--space-8) var(--space-4)', textAlign: 'center', color: 'var(--text-tertiary)' }}>
                    ...
                    <div style={{ fontSize: 'var(--fs-body)', fontWeight: 500 }}>Retrieving biller directory…</div>
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} style={{ padding: 'var(--space-8) var(--space-4)', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 'var(--fs-body)' }}>
                    {(search || hasPeriod)
                      ? <>No biller matches the current filters{search && <> for <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>"{search}"</span></>}.</>
                ...
                <tr key={idx} style={{ borderTop: '1px solid var(--border-faint)' }}>
                  ...
                  <Td strong>{a.BILLER_NAME}</Td>
                  <Td mono>{a.ACCOUNT_ID}</Td>
                  <Td mono>{a.BILLER_SHORT_CODE}</Td>
                  <Td muted mono>{a.IDENTITY_ID}</Td>
                  <Td>{a.DEPARTMENT}</Td>
                  <Td muted>{a.BILLER_TYPE}</Td>
