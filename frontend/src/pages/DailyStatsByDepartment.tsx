...
import { useDailyStatsByDepartment } from "../hooks/useAnalytics"
...
function buildPivot(rows: any[], metric: Metric): Pivot {
  ...
  for (const r of rows) map.set(`${r.DEPARTMENT}|${normDate(r.TRANSACTION_DATE)}`, r)

  const pivotRows = depts.map(d => {
    const cells = dates.map(dt => Number((map.get(`${d}|${dt}`) || {})[metric] ?? 0))
...
export default function DailyStatsByDepartment() {
  ...
  const { data: rows, isLoading, isFetching, refetch } = useDailyStatsByDepartment({
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const res = await fetch(`${OpenAPI.BASE}/customers/daily-stats-by-department/export.xlsx?${qs.toString()}`)
      ...
      a.href = dl
      a.download = `Daily_Stats_by_Department_${submittedFilters.startDate}_to_${submittedFilters.endDate}.xlsx`
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{
            ...
            Volume, subs, value and revenue aggregated by day × DEPARTMENT · {submittedFilters.startDate} → {submittedFilters.endDate}
          </p>
        </div>
      ...
      <div style={{
        ...
        {/* TR_TYPE filter — restrict which products feed the per-department aggregates */}
      ...
      {/* ── METRIC TABS + PIVOT — DEPARTMENT en lignes, dates en colonnes ── */}
...
// ── Pivot Table component — DEPARTMENT en lignes, dates en colonnes ──
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          {pivot.rows.length} department{pivot.rows.length > 1 ? 's' : ''} · {pivot.dates.length} day{pivot.dates.length > 1 ? 's' : ''}
        </span>
      </div>
