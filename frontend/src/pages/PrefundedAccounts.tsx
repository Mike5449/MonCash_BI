...
import { Banknote, Download, Search, RefreshCw, Calendar, X } from "lucide-react"
import "../premium.css"
    ...
    <DashboardLayout>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ flex: 1 }}>
            <span style={{ display: 'block', fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--mc-red)', letterSpacing: '0.05em' }}>Prefunded Repository</span>
      ...
      <div className="table-card animate-fade-in" style={{ marginTop: '24px' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--mc-border)', background: '#f8fafc' }}>
           <h3 style={{ fontSize: '13px', fontWeight: 800, margin: 0, textTransform: 'uppercase', color: '#64748b' }}>
             Registered Prefunded Partners{hasFilter ? ` · filtered (${totalShown})` : ` (Top ${totalLoaded || 100})`}
           </h3>
        </div>
          ...
          <table className="mc-table" style={{ minWidth: '1300px' }}>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '100px', color: '#94a3b8' }}>
                  {hasFilter
                    ? `No partner matches the current filters${search ? ` for "${search}"` : ""}.`
